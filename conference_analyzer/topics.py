...
_ABSTRACT_CHARS = 700
_ASSIGN_BATCH = 25
_MAX_TOPICS_PER_PAPER = 2


def _theme_block(theme: str, theme_definition: str = "") -> str:
    block = f'Theme: "{theme}"'
    if theme_definition and theme_definition.strip():
        block += f"\nScope of the theme (what counts / doesn't): {theme_definition.strip()}"
    return block
...
_ASSIGN_SYSTEM = (
    "You assign each paper to the topic(s) it fits from a fixed list. Pick the single "
    "best-fitting topic (the primary) by the paper's core contribution. Add a SECOND "
    "topic only if the paper substantially contributes to it as well — most papers get "
    "one topic. Return `topic_ids` (one or two topic ids), best-fitting first."
)
...
def _discover_topics(
    client: LLMClient, theme: str, papers: list[Paper], n_topics: int, theme_definition: str = ""
) -> list[Topic]:
    lines = [
        _theme_block(theme, theme_definition),
...
def _assign_topics(
    ...
    for start in range(0, total, _ASSIGN_BATCH):
        ...
        by_index = {a["index"]: a.get("topic_ids", []) for a in data.get("assignments", [])}
        valid_ids = {t.topic_id for t in topics}
        for i, paper in enumerate(batch):
            ids: list[int] = []
            for tid in by_index.get(start + i) or []:
                if tid in valid_ids and tid not in ids:
                    ids.append(tid)
                if len(ids) >= _MAX_TOPICS_PER_PAPER:
                    break
            paper.topic_ids = ids or [topics[0].topic_id]
        done += len(batch)
        if progress:
...
def model_topics_llm(
    ...
    if not papers:
        return []
    topics = _discover_topics(client, theme, papers, n_topics, theme_definition)
    if not topics:
    ...
    for p in papers:
        for tid in p.topic_ids:
            if tid in by_id:
                by_id[tid].paper_ids.append(p.paper_id)
    ...
    for p in papers:
        p.topic_ids = [remap[t] for t in p.topic_ids if t in remap]
...
def model_topics(
    ...
    return model_topics_llm(client, theme, papers, n_topics, progress, theme_definition)
...
def _summary_prompt(theme: str, topic: Topic, papers: list[Paper], theme_definition: str = "") -> str:
    lines = [
        _theme_block(theme, theme_definition),
        f'Topic: "{topic.name}"',
        "",
...
def _summary_cache_path(cache_dir: str, cache_sig: str, theme: str, theme_definition: str) -> str:
    digest = hashlib.sha1(
        f"{cache_sig}|{theme}|{theme_definition}|topicsum".encode("utf-8")
    ).hexdigest()[:16]
...
def summarize_topics(
    ...
    if cache_dir:
        ...
        cache_path = _summary_cache_path(cache_dir, cache_sig, theme, theme_definition)
    ...
    for i, topic in enumerate(topics):
        ...
        if hit:
            ...
            try:
                data = client.structured(
                    _SUMMARY_SYSTEM,
                    _summary_prompt(theme, topic, batch, theme_definition),
                    _SUMMARY_SCHEMA,
                    max_tokens=1500,
                    effort="medium",
                )
